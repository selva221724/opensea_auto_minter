...
def main_program_loop():
    ...
    import glob
    imgs = glob.glob(pathname=r"C:\Users\TamilS\Documents\stack\color\items"+'\*.png')
    print(imgs[0])
    print('got the text')
    for i in imgs:
    # while end_num >= start_num:
        ...
        name.send_keys(Path(i).name.split('.')[0])  # +1000 for other folders #change name before "#"
        time.sleep(0.5)
        ...
        desc.send_keys("This is a named color collectible with a shape of 3000x3000 pixels.Each Collectible represents the "
                       "unique colors that can project ourself in meta-life")
        time.sleep(0.5)
